...
MSG_TXT = '{"deviceId": "Raspberry Pi - Python","temperature": %f,"humidity": %f,"pressure": %f, "timestampRasp": %f}'
    ...
    try:
        ...
        if client.protocol == IoTHubTransportProvider.MQTT:
            ...
            client.send_reported_state(
                reported_state, len(reported_state), send_reported_state_callback, SEND_REPORTED_STATE_CONTEXT
            )
        ...
        while True:
            global MESSAGE_COUNT, MESSAGE_SWITCH, temperature, humidity, pressure
            if MESSAGE_SWITCH:
                ...
                msg_txt_formatted = MSG_TXT % (temperature, humidity, pressure, time.time())
        ...
        telemetry.send_telemetry_data(
            parse_iot_hub_name(), EVENT_FAILED, "Unexpected error %s from IoTHub" % iothub_error
        )
        return
...
def main():
    ...
    client.connect(BROKER_IP, 1883, 60)  # connect
